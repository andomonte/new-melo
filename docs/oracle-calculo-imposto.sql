-- =====================================================================
-- DOCUMENTAÇÃO (extraída do banco DEV — DESENV.MNS.MELOPECAS.COM.BR)
-- =====================================================================
-- SEGURANÇA (item 0):
--   * Banco = DEV/cópia (DB_NAME='desenv', host 'srvdesenvmao'), NÃO produção.
--   * CALCULO_IMPOSTO (body 3179 linhas): NENHUM INSERT/UPDATE/DELETE/MERGE
--     — é cálculo puro (sem efeito colateral). Seguro para executar.
--
-- PONTO DE ENTRADA / COMO CHAMAR (convenção do caller Delphi):
--   1) CALCULO_IMPOSTO.INICIALIZACAO(Tipo_Movimentacao, TipoOperacao,
--         CodProduto, Codigo(=codcli), CodigoTerceiro)  -- carrega estado:
--         RowUF_Origem/Destino, CIDADE_*, DadosDestino, ProdImportado, etc.
--   2) Aliquota_ICMS := GET_ICMS(Tipo_Mov, TipoOp, CodProd, Codigo, Insc_Estadual, CFOP)
--      Aliquota_IPI  := GET_IPI (Tipo_Mov, TipoOp, CodProd, Codigo)
--      -> A PROCEDURE NÃO DERIVA as alíquotas: o caller as obtém e passa.
--   3) CALCULO_IMPOSTO.CALCULAR_IMPOSTOS(...121 parâmetros...)
--
-- PARÂMETROS DE CALCULAR_IMPOSTOS (121):
--   IN (1-9): Tipo_Movimentacao, TipoFatura, TipoOperacao, Zerar_SUBSTITUICAO,
--             Aliquota_IPI, Aliquota_ICMS, Total_Produto, Base_Produto, MVA_ANTECIPADO
--   IN OUT (10-29, clássicos): NCM, Aliquota_Pis, Aliquota_Cofins, Base_Pis,
--             Base_Cofins, Valor_Pis, Valor_Cofins, CstPis, CstCofins, CstIPI,
--             Base_IPI, Valor_IPI, Base_Calc_ICMS, Valor_ICMS,
--             Base_Calc_ICMS_Subst, Valor_ICMS_Subst, MVA,
--             ICMS_Interno_Destino, ICMS_Externo_Origem, CFOP
--   IN OUT (30-121): campos da Reforma Tributária IBS/CBS (gibscbs_*, mono*,
--             trib*, credpres*, is_*) — saída informativa da reforma.
--   Valores esperados: Tipo_Movimentacao ∈ {SAIDA, ENTRADA, ENTRADA_COMPRAS};
--             TipoOperacao ∈ {VENDA, TRANSFERENCIA, DEVOLUCAO_COMPRA,
--             DEVOLUCAO_TRANSFERENCIA, REMESSA_BONIFICACAO, REMESSA_EXPOSICAO,
--             REMESSA_DEMOSTRACAO, ...}.
--
-- SEMÂNTICA RELEVANTE (achados de código, ainda não executado):
--   * IPI = Base_Produto * Aliquota_IPI/100 (linha ~622) — vem do caller (GET_IPI).
--   * ICMS: ICMS_Interno_Destino = RowUF_Destino.Icmsinterno; se ProdImportado
--     então ICMS_Externo_Origem = 4.00; se cliente final + IE 'ISENTO' -> zera
--     ICMS/MVA; Zerar_SUBSTITUICAO='S' -> zera (Calcular_ICMS_Subst).
--   * Desconto: a base de ICMS usa Total_Produto (input do caller); logo se o
--     desconto reduz a base depende do que o Delphi passa em Total_Produto/Base_Produto.
--
-- TABELAS/VIEWS CONSULTADAS (só SELECT):
--   cad_legislacao_icmsst_ncm, cad_legislacao_icmsst, dbclien, dadosempresa,
--   dbprod, dbclassificacao_piscofins, dbclassificacao_protocolo129,
--   dbclassificacao_stsankhya, dbcredor, cad_credor_regra_faturamento,
--   dbcfop_n, dbcfop, zfm_combinacoes, alc_combinacoes (ZFM/ALC = Suframa), dual.
-- =====================================================================

-- ============ PACKAGE CALCULO_IMPOSTO (SPEC) ============
PACKAGE CALCULO_IMPOSTO is
 ...
 PROCEDURE Calcular_IBS_CBS(TipoOperacao in varchar2,                             
                         ...
                         vtpcredpresibszfm in out number,
                        indbemmovelusado in out number,
                        is_cstis in out varchar2,
                        is_cclasstribis in out varchar2,
                        is_vbcis in out number,
                        is_pis in out number,
                        is_pisexpec in out number,
                        is_utrib in out varchar2,
                        is_qtrib in out number,
                        is_vis in out number,
                        ibscbs_cst in out varchar2,
                        ibscbs_cclasstrib in out varchar2,
                        ...
                        gibscbs_gcbs_paliqefet in out number,
                        gibscbs_gcbs_vcbs in out number,
                        tribregular_cstreg in out varchar2,
                        tribregular_cclasstribreg in out varchar2,
                        tribregular_paliqefetregibsuf in out varchar2,
                        tribregular_vtribregibsuf in out number,
                        tribregular_paliqefetregibsmun in out number,
                        tribregular_vtribregibsmun in out number,
                        tribregular_paliqefetregcbs in out number,
                        ...
                        transfcred_vibs in out number,
                        transfcred_vcbs in out number,
                        ajustecompet_competapur in out varchar2,
                        ajustecompet_vibs in out number,
                        ajustecompet_vcbs in out number,
                        estornocred_vibsestcred in out number,
                        estornocred_vcbsestcred in out number,
                        ...
                        cbscredpres_vcredpres in out number,
                        cbscredpres_vcredprescondsus in out number,
                        credpresibszfm_competapur in out varchar2,
                        credpresibszfm_tpcredpresibszf in out number,
                        credpresibszfm_vcredpresibszfm in out number,
                        vitem in out number,
                        dfereferenciado_chaveacesso in out varchar2,
                        dfereferenciado_nitem in out number);
 ...
 begin
    ...
    if (TipoFatura = 'NOTA_FISCAL') or (Tipo_Movimentacao = 'ENTRADA_COMPRAS')
      then -- Nota Fiscal
        ...
        if xDadosEmpresa.uf = 'RO' and  DadosDestino.Codigo = '00169' and CIDADE_Destino.Uf = xDadosEmpresa.uf
          then
            xPode_ST := False;
            if (Tipo_Movimentacao = 'ENTRADA') or (Tipo_Movimentacao = 'ENTRADA_COMPRAS')
              then -- FATURAMENTO - Nota de Entrada
...
dfereferenciado_nitem);
 ...
 BEGIN
   ...
   xAliquota_CBS  := round(0.9, 4);
    ...
    /*elsif TipoOperacao = 'REMESSA_BONIFICACAO'
      ...
      then
        ibscbs_cst := '410';
        ibscbs_cclasstrib := '410999';
    elsif CIDADE_Origem.Codmunicipio not in (1100106,1200104,1200203,1200252,1304062,1400100,1400159,1400456,1600303,1600600/*'BRASILEIA','EPITACIOLANDIA','TABATINGA','GUAJARA-MIRIM','BOA VISTA','BONFIM','MACAPA','SANTANA'*/) and 
    CIDADE_Destino.Codmunicipio in (1100106,1200104,1200203,1200252,1304062,1400100,1400159,1400456,1600303,1600600/*'BRASILEIA','EPITACIOLANDIA','TABATINGA','GUAJARA-MIRIM','BOA VISTA','BONFIM','MACAPA','SANTANA'*/)
      then
        ibscbs_cst := '200';
        ibscbs_cclasstrib := '200024';
        --ibscbs_inddoacao in out number,
        gibscbs_vbc := Total_Produto;
        gibscbs_gibsuf_pibsuf := 0.0000;
        --gibscbs_gibsuf_pdif in out number,
        --gibscbs_gibsuf_vdif in out number,
        --gibscbs_gibsuf_vdevtrib in out number,
        gibscbs_gibsuf_predaliq := round(100, 4);
        gibscbs_gibsuf_paliqefet := round(0, 4);
        gibscbs_gibsuf_vibsuf := round(0 / 100, 2);
        gibscbs_gibsmun_pibsmun := 0.0000;
        --gibscbs_gibsmun_pdif in out number,
        --gibscbs_gibsmun_vdif in out number,
        --gibscbs_gibsmun_vdevtrib in out number,
        gibscbs_gibsmun_predaliq := round(100, 4);
        gibscbs_gibsmun_paliqefet := round(0, 4);
        gibscbs_gibsmun_vibsmun := round(0/ 100, 2);
        gibscbs_vibs := round(0, 2);
        gibscbs_gcbs_pcbs := 0;
        --gibscbs_gcbs_pdif in out number,
        --gibscbs_gcbs_vdif in out number,
        --gibscbs_gcbs_vdevtrib in out number,
        gibscbs_gcbs_predaliq := round(100, 4);
        gibscbs_gcbs_paliqefet := round(0, 4);
        gibscbs_gcbs_vcbs := round(0 / 100, 2);
    ...
    elsif CIDADE_Origem.Codmunicipio not in (9999999,1302603,1303569,1303536) and CIDADE_Destino.Codmunicipio in (1302603,1303569,1303536)/*('MANAUS','RIO PRETO DA EVA','PRESIDENTE FIGUEIREDO')*/
      then
        ibscbs_cst := '200';
 ...
 BEGIN
   ...
   elsif xDadosEmpresa.uf = 'RO' and  DadosDestino.Codigo = '00169' and CIDADE_Destino.Uf = xDadosEmpresa.uf
       then
         if ProdImportado
           then
             xResult := 4.00;
...
-- ============ PACKAGE CFOP (BODY) ============
package body CFOP is

  /* inclui novo CFOP - codigo fiscal de operac?o */
 PROCEDURE Inc_CFOP(vdescr in DBCFOP.descr%type, 
                    vicms_fora in DBCFOP.icms_fora%type,
                    vicms_am in DBCFOP.icms_am%type,
                    vcfop in DBCFOP.cfop%type) is
  --declare
  nro number := 0;
  nro_reg number;
 BEGIN  
  Insert Into DBCFOP(CFOP,Descr,ICMS_Fora,ICMS_Am)
              Values(vCFOP,vDescr,vICMS_Fora,vICMS_Am);
 END Inc_CFOP;
 
 /* altera dados do CFOP */
 PROCEDURE Alt_CFOP(vdescr in DBCFOP.descr%type, 
                    vicms_fora in DBCFOP.icms_fora%type,
                    vicms_am in DBCFOP.icms_am%type,
                    vcfop in DBCFOP.cfop%type) is
 BEGIN
  ...
  UpDate DBCFOP 
     Set Descr=vDescr, 
         ICMS_Fora=vICMS_Fora, 
         ICMS_Am=vICMS_Am
  Where CFOP = vCFOP;
 END Alt_CFOP; 
 
 PROCEDURE Nvg_CFOP(cursor_cfop out cursorgenerico.tipocursorgenerico) is
 BEGIN
  Open cursor_cfop for
   Select * from DBCFOP;
 END;
 
 PROCEDURE Del_CFOP(vcfop in DBCFOP.cfop%type) is
 BEGIN
  Delete from DBCFOP where cfop=vcfop;
 END;

end CFOP;


-- ============ PACKAGE CFOP_NOVA (BODY) ============
package body CFOP_NOVA is

  /* inclui novo CFOP - codigo fiscal de operacao */
 PROCEDURE INC_CFOP_N(vCFOP in DBCFOP_N.CFOP%type,
                      vDescr in DBCFOP_N.descr%type,                      
                      vCFOPInverso in DBCFOP_N.CFOPINVERSO%type) is
  --declare
  nro number := 0;
  nro_reg number;
 BEGIN  
  Insert Into DBCFOP_N(CFOP,Descr,CFOPINVERSO)
              Values(vCFOP,vDescr,vCFOPInverso);
 END Inc_CFOP_N;
 
 /* altera dados do CFOP */
 PROCEDURE Alt_CFOP_N(vCFOP in DBCFOP_N.CFOP%type,
                      vDescr in DBCFOP_N.descr%type, 
                      vCFOPINVERSO in DBCFOP_N.CFOPInverso%type) is
 BEGIN
  --atualiza dados
  UpDate DBCFOP_N 
     Set Descr=vDescr, 
         cfopinverso = vcfopinverso
  Where CFOP = vCFOP;
 END Alt_CFOP_N; 
 
 PROCEDURE Nvg_CFOP_N(cursor_cfop_n out cursorgenerico.tipocursorgenerico) is
 BEGIN
  Open cursor_cfop_n for
   Select * from DBCFOP_N Order by cfop;
 END;
 
 PROCEDURE Del_CFOP_N(vCFOP in DBCFOP_N.CFOP%type) is
 BEGIN
  Delete from DBCFOP_N where cfop=vcfop;
 END;end CFOP_NOVA;
