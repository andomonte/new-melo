// Script corrigido para extrair chave NFe corretamente e fixar QR Code
// Problema identificado: chave está no atributo Id, não em tag separada
...
class QRCodeFixer {
  ...
  // Carrega envelope e extrai chave corretamente
  loadEnvelope(envelopeData) {
    ...
    if (chaveMatch) {
      ...
      console.log(`🔑 Chave NFe extraída do Id: ${this.chNFe}`);
    } else {
      console.log('❌ Chave NFe não encontrada no atributo Id');
    }
  ...
  fixQRCode() {
    ...
    // Construir string do QR Code sem o hash primeiro
    const qrStringWithoutHash = this.buildQRStringWithoutHash(params);

    // Calcular hash correto (32 caracteres)
    const hash = this.calculateQRHash(qrStringWithoutHash);
  ...
  // Constrói string do QR Code sem o hash
  ...
  // Constrói string completa do QR Code
  ...
  runFix() {
    ...
    try {
      ...
      if (fixed) {
        ...
        console.log('   - Chave NFe extraída corretamente do atributo Id');
        console.log('   - Hash recalculado com tamanho correto (32 caracteres)');
        console.log('   - QR Code reconstruído com todos os parâmetros corretos');
...
function main() {
  ...
  if (args.length === 0) {
    console.log('📖 Uso: node qr-fixer-fixed.js <arquivo.xml>');
    console.log('');
    console.log('Exemplos:');
    console.log('  node qr-fixer-fixed.js envelope-exemplo.xml');
    console.log('');
    console.log('O script irá:');
    console.log('  1. Extrair chave NFe do atributo Id (não de tag separada)');
    console.log('  2. Analisar o QR Code atual');
    console.log('  3. Recalcular hash com tamanho correto (32 caracteres)');
