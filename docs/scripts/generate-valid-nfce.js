// Script para gerar XML NFC-e mais completo e válido
// Baseado em schemas oficiais do SEFAZ
  ...
  async loadData() {
    const query = `
      SELECT
        "certificadoKey",
        "certificadoCrt",
        csc_nfce_producao as csc_token,
        csc_nfce_id,
        nomecontribuinte,
        cgc,
        inscricaoestadual,
    ...
    this.cscToken = await decrypt(row.csc_token);
    this.cscId = row.csc_nfce_id;

    return row;
  }

  // Gerar XML NFC-e válido conforme schema
  generateValidNFCe(empresaData) {
    const now = new Date();
    const dhEmi = now.toISOString().replace('T', ' ').substring(0, 19) + '-03:00';
    const serie = '1';
    const nNF = Math.floor(Math.random() * 999999).toString().padStart(6, '0');
...
<xFant>${empresaData.nomecontribuinte.substring(0, 60)}</xFant>
<enderEmit>
...
<email>suporte@empresa.com</email>
<fone>92999999999</fone>
</infRespTec>
<infNFeSupl><qrCode><![CDATA[${qrCode}]]></qrCode></infNFeSupl>
</infNFe>
</NFe>`;
  ...
  async signNFCeXML(xml) {
    console.log('🔐 Assinando XML NFC-e válido...');
    ...
    try {
      ...
      console.log('✅ XML válido assinado com sucesso');
      return xmlAssinado;
...
<X509Certificate>${Buffer.from(this.certificadoCrt).toString('base64')}</X509Certificate>
  ...
  // Executar geração
  async generateValidXML() {
    console.log('🏗️  GERANDO XML NFC-e VÁLIDO');
    console.log('==============================');

    try {
      await this.connect();
      const empresaData = await this.loadData();

      console.log(`🏢 Empresa: ${empresaData.nomecontribuinte}`);
      console.log(`🏷️  CNPJ: ${empresaData.cgc}`);
      console.log(`🔢 CSC ID: ${this.cscId}`);

      // Gerar XML válido
      const { xml, chaveAcesso, qrCode } = this.generateValidNFCe(empresaData);
      ...
      // Salvar XML assinado
      const fs = require('fs');
      fs.writeFileSync('nfce-valido-assinado.xml', xmlAssinado);
      console.log('💾 XML válido assinado salvo em: nfce-valido-assinado.xml');

      return { success: true, xml: xmlAssinado, chaveAcesso, qrCode };
...
// Executar geração
async function main() {
  const generator = new ValidNFCeGenerator();
  await generator.generateValidXML();
}
