// Script para testar NFC-e completo com assinatura correta
// Usa o XML assinado do teste anterior
...
class CompleteNFCeTester {
  ...
  // Criar envelope SOAP NFC-e
  createSOAPEnvelope(xmlAssinado) {
    // Para NFC-e, pode ser necessário usar enviNFe como NF-e
    // Vamos testar com enviNFe
    let xmlSemDeclaracao = xmlAssinado.replace(/<\?xml[^>]*\?>/, '').trim();

    // Remover namespace duplicado do NFe (já está no enviNFe)
    xmlSemDeclaracao = xmlSemDeclaracao.replace(
...
${xmlSemDeclaracao}
  ...
  async testCompleteNFCe() {
    console.log('🚀 TESTE NFC-e COMPLETO COM ASSINATURA');
    console.log('=====================================');
    ...
    try {
      ...
      // Salvar envelope
      fs.writeFileSync('envelope-completo.xml', envelope);
      console.log('💾 Envelope salvo em: envelope-completo.xml');
  ...
  async runTest() {
    ...
    if (result.success) {
      ...
      console.log('💡 Verificar envelope-completo.xml para detalhes');
    ...
    console.log('\n📋 PRÓXIMOS PASSOS:');
    console.log('   1. 🔍 Comparar envelope-completo.xml com produção');
...
// Executar teste
async function main() {
  const tester = new CompleteNFCeTester();
