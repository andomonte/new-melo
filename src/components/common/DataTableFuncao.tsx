...
export default function DataTable({
  ...
  const calculateColumnWidth = (index: number, totalColumns: number) => {
    if (totalColumns === 4) {
      if (index === 0) {
        return '45%'; // Largura para a primeira coluna (aproximadamente 3 vezes as outras)
      } else {
        return '18.33%'; // Largura para as outras três colunas (dividindo o restante)
      }
    }
    // Lógica original para outros cenários (manter como fallback)
