...
export default function DataTable({
  ...
  const renderColGroup = () => {
    ...
    const remainingCols = totalCols - 1; // porque a primeira ocupa 30%

    return (
      <colgroup>
        {headers.map((_, index) => {
          if (index === 0) {
            return <col key={index} style={{ width: '30%' }} />;
          } else {
            const colWidth = (70 / remainingCols).toFixed(2);
            ...
            {!Array.isArray(rows) ? (
              ...
              <TableRow>
                <TableCell
                  ...
                  style={{ height: 'calc(100vh - 16rem - 4rem)' }} // ajuste conforme sua estrutura real
                >
                  Não temos promoções ativas.
                </TableCell>
              </TableRow>
            ) : (
