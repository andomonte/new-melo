...
import { Meta } from '../../data/common/meta'; // Certifique-se que o caminho está correto
...
interface DataTableProps {
  ...
  loading: boolean; // ADICIONADO: Propriedade para controlar o estado de carregamento
  noDataMessage?: string; // ADICIONADO: Mensagem customizável para quando não há dados
}

export default function DataTable({
  headers,
  rows = [], // CORREÇÃO: Valor padrão para 'rows' para garantir que seja sempre um array
  meta,
  onPageChange,
  onPerPageChange,
  onSearch,
  ...
  loading, // DESESTRUTURADO: A prop 'loading'
  noDataMessage = 'Nenhum dado encontrado.', // DESESTRUTURADO: A prop 'noDataMessage' com valor padrão
}: DataTableProps) {
    ...
    <div className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-gray-300 rounded-lg overflow-hidden max-w-full flex flex-col h-[calc(100vh-10rem)]">
      ...
      <div className="h-full flex-grow overflow-y-auto py-0">
        <Table className="min-w-full table-auto text-sm text-center text-gray-700 dark:text-gray-200">
          {renderColGroup()}
          <TableBody>
            {loading ? ( // CORREÇÃO: Exibe Carregamento se 'loading' for true
              <TableRow>
                <TableCell
            ...
            ) : rows.length === 0 ? ( // CORREÇÃO: Exibe mensagem de sem dados se não estiver carregando E 'rows' estiver vazio
              <TableRow>
                <TableCell
                  colSpan={headers.length}
              ...
              rows.map((row, rowIndex) => (
                ...
                >
                  {/* Assumindo que 'row' é um objeto e você quer renderizar todos os seus valores */}
