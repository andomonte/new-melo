...
interface DataTableNavegavelProps {
  headers: string[];
  rows: any[];
  meta: Meta;
  ...
  /** Chamado quando ↑ na primeira linha — permite o pai focar em outro elemento */
  onNavigateUp?: () => void;
}

export default function DataTableNavegavel({
  headers,
  rows,
  meta,
  onPageChange,
  ...
  const highlightRow = (rowIdx: number) => {
    ...
    tbody.querySelectorAll('tr[data-nav-row].nav-focused').forEach(tr => {
      (tr as HTMLElement).classList.remove('nav-focused', 'ring-2', 'ring-inset', 'ring-blue-400/60', 'bg-blue-50/50', 'dark:bg-blue-950/30');
    });
    // Adicionar highlight novo
    if (rowIdx >= 0) {
      const tr = tbody.querySelectorAll('tr[data-nav-row]')[rowIdx] as HTMLElement;
      if (tr) tr.classList.add('nav-focused', 'ring-2', 'ring-inset', 'ring-blue-400/60', 'bg-blue-50/50', 'dark:bg-blue-950/30');
    }
  };

  const setFocusedRow = (val: number) => {
    focusedRowRef.current = val;
    highlightRow(val);
  };
  ...
  // ===== NAVEGAÇÃO POR TECLADO (handlers) =====
  const scrollRowIntoView = (rowIdx: number) => {
    ...
    const tr = tbodyRef.current.querySelectorAll('tr[data-nav-row]')[rowIdx] as HTMLElement;
    if (!tr) return;
    const trRect = tr.getBoundingClientRect();
    const contRect = containerRef.current.getBoundingClientRect();
    if (trRect.top < contRect.top) {
      containerRef.current.scrollTop -= (contRect.top - trRect.top);
    } else if (trRect.bottom > contRect.bottom) {
      containerRef.current.scrollTop += (trRect.bottom - contRect.bottom);
    }
  };
  ...
  const handleTableKeyDown = (e: React.KeyboardEvent) => {
    ...
    const totalRows = linhasOrdenadas?.length || 0;
    if (totalRows === 0) {
      // Grid vazio — ↑ volta para busca, qualquer outra tecla ignora
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        const searchInput = document.querySelector('[data-datatable-search] input') as HTMLElement;
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      const next = currentRow < 0 ? 0 : Math.min(currentRow + 1, totalRows - 1);
      setFocusedRow(next);
      setTimeout(() => scrollRowIntoView(next), 0);
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (currentRow <= 0) {
        // Sair do grid — focar no input de busca (linha 2)
        const searchInput = document.querySelector('[data-datatable-search] input') as HTMLElement;
        if (searchInput) searchInput.focus({ preventScroll: true });
        return;
      }
      ...
      e.preventDefault();
      e.stopPropagation();
      // Delay para evitar que o Enter borbulhe para o menu que vai abrir
      setTimeout(() => {
        const tbody = tbodyRef.current;
        if (tbody) {
          const tr = tbody.querySelectorAll('tr[data-nav-row]')[currentRow] as HTMLElement;
          if (tr) {
            const actionBtn = tr.querySelector('button[data-action-toggle]') as HTMLButtonElement;
            if (actionBtn) {
              actionBtn.click();
            }
          }
    ...
    <div className="relative border border-gray-300 dark:border-gray-300 bg-white dark:bg-zinc-900 rounded-lg flex flex-col flex-1 min-h-0 overflow-hidden shadow-sm">
      ...
      <div className="border-b border-gray-200 dark:border-zinc-700 p-2">
        <div className="flex items-center gap-3 flex-wrap">
          {headerLeftSlot}
          <div data-datatable-search className={searchCompacto ? 'w-56 sm:w-72 shrink-0 ml-auto' : 'flex-1 min-w-0'}>
          <SearchInput
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2 py-1 border rounded-md bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-xs text-gray-700 dark:text-white"
                >
      ...
      <div className="relative flex-1 min-h-0 overflow-auto" ref={containerRef}>
          ...
          <div className="min-w-max">
            ...
            <tbody
              ...
              className="bg-white dark:bg-zinc-900 divide-y divide-gray-200 dark:divide-zinc-700 outline-none"
            >{
              rows?.length === 0 ? (
                null
              ) : (
