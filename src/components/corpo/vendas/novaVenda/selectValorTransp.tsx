...
const SelectVendedor: React.FC<ChildProps> = ({
  ...
  dadosTransporte.map((val) => {
    const newListaVend = createDadosVendedorSel(val.DESCR, val.CODTPTRANSP);
              ...
              {arrayListaVendedor.map((framework) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    setOpen(false);
                    const transSelect = dadosTransporte.filter(
                      (val) => val.CODTPTRANSP === framework.value,
                    );

                    if (transSelect.length) handleTransporteSel(transSelect[0]);
                  }}
                >
                  {framework.label}
                  <Check
