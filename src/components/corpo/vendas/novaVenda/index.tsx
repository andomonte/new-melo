...
export default function HomeVenda() {
  ...
  // Flags de liberação pelo gerente (impede recálculo automático de bloqueio)
  const [descLiberado, setDescLiberado] = React.useState(false);
  const [finLiberado, setFinLiberado] = React.useState(false);
  ...
  const [cliente, setCliente] = React.useState<
    { codigo: string; nome: string; documento: string; nomeFantasia: string }[]
  >([{ codigo: '01', nome: '', documento: '', nomeFantasia: '' }]);
  ...
  // Estado de kickback - usado APENAS na tela de produtos (antes de ir pro carrinho)
  // No carrinho, usa item.usandoKickback como fonte de verdade
  const [kickbackMarcadoPorProduto, setKickbackMarcadoPorProduto] = React.useState<Record<string, boolean>>({});
  const [precosOriginaisKickback, setPrecosOriginaisKickback] = React.useState<Record<string, string>>({});
    ...
    try {
      ...
      const payload = {
        ...
        header: {
          ...
          // Campos de bloqueio - preservar status ao salvar
          statusVenda: statusVenda || 'VENDA LIBERADA',
          blocFin: !!blocFin,
          blocDesc: !!blocDesc,
          // Flags de liberação pelo gerente
          descLiberado: !!descLiberado,
          finLiberado: !!finLiberado,
        },
  ...
  React.useLayoutEffect(() => {
    try {
      ...
      // Restaurar campos de bloqueio (importante para vendas salvas bloqueadas)
      if (meta.statusVenda) setStatusVenda(meta.statusVenda);
      if (typeof meta.blocFin === 'boolean') setBlocFin(meta.blocFin);
      if (typeof meta.blocDesc === 'boolean') setBlocDesc(meta.blocDesc);
      // Restaurar flags de liberação pelo gerente
      if (typeof meta.descLiberado === 'boolean')
        setDescLiberado(meta.descLiberado);
      if (typeof meta.finLiberado === 'boolean')
        setFinLiberado(meta.finLiberado);
  ...
  React.useEffect(() => {
    ...
    const metaParcial = {
      ...
      // Campos de bloqueio - preservar status da venda
      statusVenda,
      blocFin,
      blocDesc,
      // Flags de liberação pelo gerente
  ...
  // Validação para FINALIZAR venda (inclui verificação de bloqueio)
  function validarObrigatorios(): string[] {
    ...
    // Estados que bloqueiam a operação (apenas BLOQUEIO FINANCEIRO impede)
    // REQUER LIBERAÇÃO (blocDesc) permite finalizar, mas vai salvar como status 'B'
    if (statusVenda === 'BLOQUEIO FINANCEIRO') {
      faltas.push('Bloqueio financeiro impede a operação');
    }

    return faltas;
  }

  // Validação para SALVAR orçamento (permite salvar vendas bloqueadas)
  function validarObrigatoriosParaSalvar(): string[] {
    const faltas: string[] = [];
    ...
    // NÃO verifica bloqueio - permite salvar vendas bloqueadas
    ...
    try {
      ...
      const payload = {
        header: {
          ...
          obs: String(obs ?? ''),
          obsfat: String(obsFat ?? ''),
          bloqueada: blocDesc ? 'S' : '0', // 'S' = requer liberação (status 'B')
          estoque_virtual: 'N',
          uName: String(perfilUser?.usuario),
          nomecf:
            clienteSelect?.nome?.trim?.() ||
        ...
        itens: (carrinho ?? []).map((val: any, idx: number) => {
          // Se temos campos completos da API, usa direto (mais confiável e completo)
          // Caso contrário, faz fallback para os campos antigos (retrocompatibilidade)
          const campos = val.campos;

          const extrasFiscais = campos
            ? {
                // Campos completos da nova API /api/impostos
                icms: campos.icms,
                baseicms: campos.baseicms,
                totalicms: campos.totalicms,
                icmsinterno_dest: campos.icmsinterno_dest,
                icmsexterno_orig: campos.icmsexterno_orig,
                csticms: campos.csticms,

                mva: campos.mva,
                basesubst_trib: campos.basesubst_trib,
                totalsubst_trib: campos.totalsubst_trib,

                ipi: campos.ipi,
                baseipi: campos.baseipi,
                totalipi: campos.totalipi,
                cstipi: campos.cstipi,

                pis: campos.pis,
                basepis: campos.basepis,
                valorpis: campos.valorpis,
                cstpis: campos.cstpis,

                cofins: campos.cofins,
                basecofins: campos.basecofins,
                valorcofins: campos.valorcofins,
                cstcofins: campos.cstcofins,

                fcp: campos.fcp,
                base_fcp: campos.base_fcp,
                valor_fcp: campos.valor_fcp,
                fcp_subst: campos.fcp_subst,
                basefcp_subst: campos.basefcp_subst,
                valorfcp_subst: campos.valorfcp_subst,

                cfop: campos.cfop,
                tipocfop: campos.tipocfop,
                ncm: campos.ncm,

                totalproduto: campos.totalproduto,

                // IBS/CBS (Reforma Tributária 2026)
                aliquota_ibs: val.aliquotas?.ibs ?? null,
                aliquota_cbs: val.aliquotas?.cbs ?? null,
                valor_ibs: val.impostos?.valorIBS ?? null,
                valor_cbs: val.impostos?.valorCBS ?? null,
                ibs_e: val.aliquotas?.ibs_e ?? null, // IBS Estadual (substitui ICMS)
                ibs_m: val.aliquotas?.ibs_m ?? null, // IBS Municipal (substitui ISS)
              }
            : (() => {
                // Fallback: calcular valor bruto (qtd × prunit) para bases fiscais
                // IMPORTANTE: Para NFe, a base de cálculo deve ser o valor bruto da mercadoria,
                // não o valor com desconto de promoção (totalItem)
                const qtdItem = toN(val?.quantidade ?? 1);
                const prunitItem = toN(val?.precoItemEditado ?? val?.preço ?? 0);
                const valorBruto = qtdItem * prunitItem; // Base fiscal correta

                return {
                  // Fallback: mapeia dos campos antigos (retrocompatibilidade)
                  icms: val.aliquotas?.icms ?? null,
                  ipi: val.aliquotas?.ipi ?? null,
                  mva: val.aliquotas?.agregado ?? null,
                  totalicms: val.impostos?.valorICMS ?? null,
                  totalipi: val.impostos?.valorIPI ?? null,
                  totalsubst_trib: val.impostos?.valorICMS_Subst ?? null,
                  valorpis: val.impostos?.valorPIS ?? null,
                  valorcofins: val.impostos?.valorCOFINS ?? null,
                  // Usar valor bruto para bases fiscais (não totalItem com desconto)
                  baseicms:
                    val.debugImposto?.baseICMS ??
                    val.debugImposto?.base_icms ??
                    valorBruto,
                  baseipi:
                    val.debugImposto?.baseIPI ??
                    val.debugImposto?.base_ipi ??
                    valorBruto,
                  basesubst_trib:
                    val.debugImposto?.baseST ?? val.debugImposto?.base_st ?? null,
                  basepis:
                    val.debugImposto?.basePIS ??
                    val.debugImposto?.base_pis ??
                    valorBruto,
                  basecofins:
                    val.debugImposto?.baseCOFINS ??
                    val.debugImposto?.base_cofins ??
                    valorBruto,
                  // totalproduto = valor bruto (base fiscal)
                  totalproduto:
                    val.debugImposto?.totalProduto ?? valorBruto,
                  icmsinterno_dest: val.debugImposto?.icmsInternoDest ?? null,
                  icmsexterno_orig: val.debugImposto?.icmsExternoOrig ?? null,
                  fcp: val.debugImposto?.fcp ?? null,
                  base_fcp: val.debugImposto?.base_fcp ?? null,
                  valor_fcp: val.debugImposto?.valor_fcp ?? null,
                  fcp_subst: val.debugImposto?.fcp_subst ?? null,
                  basefcp_subst: val.debugImposto?.basefcp_subst ?? null,
                  valorfcp_subst: val.debugImposto?.valorfcp_subst ?? null,
                  cfop: val.cfop ?? null,
      ...
      //      const { data } = await api.post('/api/vendas/finalizarVenda', payload);
  ...
  const refreshItemPreco = React.useCallback(
    async (item: any, armIdFallback?: number | string) => {
      ...
      try {
        // Versão ORACLE (deixe comentada se não usar):
        // const resp = await api.post('/api/vendas/oracle/produto', {
        //   descricao: chave,
        //   arm_id,
        //   pagina: 0,
        //   tamanhoPagina: 1,
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        // const r = await api.post('/api/vendas/oracle/buscarCliente', { descricao: codcli });
  ...
  const handleBuscarProd = async () => {
    ...
    await api
      //  .post('/api/vendas/oracle/produto', {
      ...
      .then((response) => {
        if (response.data.data?.length) {
          ...
          const arrayCliente = response.data.data.map((val: any) =>
            createCliente(val.CODCLI, val.NOME, val.CPFCGC, val.NOMEFANT),
          );
  ...
  }, [pesquisaCli, pagina, tamanhoPagina]);
                ...
                response.data.map((val: any) => {
                  if (val.CODPROD && val.CODPROD !== newItemProd.CODPROD) {
                    const newPerfil: any = {
                      ...
                      PRECO_KICKBACK: val.PRECO_KICKBACK ?? null,
                      PRECOVENDA: val.PRECOVENDA ?? null,
                    };
  ...
  // ========== REVALIDAÇÃO DE CLIENTE (para vendas reabertas) ==========
  /**
   * Revalida os dados do cliente buscando informações atualizadas do banco.
   * Usado quando uma venda salva é reaberta da Central de Vendas.
   */
  const revalidarCliente = async (clienteAtual: typeof clienteSelect) => {
    if (!clienteAtual || !clienteAtual.codigo) {
      console.warn('revalidarCliente: cliente inválido ou sem código');
      return;
    }

    try {
      // 1️⃣ Busca SALDO atualizado (crédito temporário ou base)
      let saldoAtualizado = 0;
      try {
        const respCredito = await api.post('/api/dbOracle/buscarCreditoTemp', {
          codClient: clienteAtual.codigo,
        });

        if (respCredito.data.length) {
          // Tem crédito temporário válido
          saldoAtualizado = respCredito.data[0].SALDO;
        } else {
          // Usa saldo base do cliente
          // Precisamos buscar LIMITE e DEBITO atualizados
          // (o clienteAtual pode ter valores desatualizados)
          saldoAtualizado = clienteAtual.saldo; // fallback
        }
      } catch (error) {
        console.error('Erro ao buscar crédito temporário:', error);
        saldoAtualizado = clienteAtual.saldo; // mantém o valor antigo em caso de erro
      }

      // 2️⃣ Calcula DIAS DE ATRASO atualizados
      let diasAtrasadoAtualizado = 0;
      try {
        const respAtraso = await api.post(
          '/api/vendas/postgresql/buscarAtraso',
          {
            codClient: clienteAtual.codigo,
          },
        );

        const d1 = respAtraso.data[0]?.DT_MIN;
        if (d1) {
          const diffInMs = Number(new Date()) - Number(new Date(d1));
          diasAtrasadoAtualizado = Math.floor(diffInMs / (1000 * 60 * 60 * 24));
        }
      } catch (error) {
        console.error('Erro ao buscar atraso:', error);
        diasAtrasadoAtualizado = clienteAtual.diasAtrasado; // mantém o valor antigo
      }

      // 3️⃣ Atualiza o clienteSelect com dados FRESCOS
      const clienteAtualizado = {
        ...clienteAtual,
        saldo: Number(saldoAtualizado.toFixed(2)),
        diasAtrasado: diasAtrasadoAtualizado,
      };

      setClienteSelect(clienteAtualizado);

      // O useEffect (linha 3380) vai disparar automaticamente e recalcular blocFin
    } catch (error) {
      console.error('Erro ao revalidar cliente:', error);
    }
  };
    ...
    await api
      ...
      .then((response) => {
        if (response.data.length) {
          ...
          saldo =
            listaCliente[Number(novoLabel)].LIMITE -
            listaCliente[Number(novoLabel)].DEBITO;
        }
      })
      .catch((error: string) => {
        console.log(error);
    ...
    if (precisaManter && clienteSelect?.codigo) {
      try {
        ...
        console.log('🔍 CÁLCULO DE IMPOSTOS:', {
          produto: novoCar.codigo,
          descricao: novoCar.descrição,
          valorUnitario: params.valorUnitario,
          quantidade: params.quantidade,
          resultadoCompleto: result,
          impostosRs: result.impostosRs,
        ...
        impostoDoItem = {
          ...
          // NOVO: campos completos para salvar na dbitvenda (inclui IBS/CBS, CSTs, bases, etc.)
          campos: result.campos,
        };
        console.log('✅ impostoDoItem montado:', impostoDoItem);
      } catch (e) {
        console.error('❌ ERRO ao calcular impostos do item:', {
          produto: novoCar.codigo,
          erro: e,
  ...
  React.useEffect(() => {
    ...
    if (resultCliSel) {
      setClienteSelect(resultCliSel);
      // ✅ REVALIDA dados do cliente (saldo e atraso atualizados)
      revalidarCliente(resultCliSel);
    }
    if (resultDadosCliSel) setDadosClienteSel(resultDadosCliSel);

    api
      .post('/api/dbOracle/buscarVendedor')
      .then((response) => {
  ...
  React.useEffect(() => {
    ...
    // Se o desconto foi liberado pelo gerente, não recalcula blocDesc como true
    if (descLiberado) {
      setBlocDesc(false);
    } else if (checkBlocDesc.length) {
      setBlocDesc(true);
    } else {
      setBlocDesc(false);
    }
  ...
  React.useEffect(() => {
    ...
    if (blocDesc && !blocFin) statusInicial = 'REQUER LIBERAÇÃO';
  ...
  React.useEffect(() => {
    ...
    if (clienteSelect && clienteSelect.codigo) {
      // Se o financeiro já liberou, não recalcula blocFin como true
      if (finLiberado) {
        if (prodInputRef.current && tela === '0') prodInputRef.current.focus();
        setBlocFin(false);
      } else if (clienteSelect.diasAtrasado <= clienteSelect.limiteAtraso) {
        if (prodInputRef.current && tela === '0') prodInputRef.current.focus();
        setBlocFin(false);
      } else {
  ...
  }, [clienteSelect, prodInputRef, tela, finLiberado]);
  ...
  function resetPersistenciaVenda(preserveEntrada: boolean = true) {
    ...
    setTransporteSel({ CODTPTRANSP: '002', DESCR: 'CARRO (MELO)' }); // objeto - valor padrão
      ...
      itensComSubtotal.map(async (item) => {
        try {
          ...
          return {
            ...
            // NOVO: campos completos para salvar na dbitvenda
            campos: result.campos,
          };
        } catch (e) {
    ...
    const novos = await Promise.all(
      carrinho.map(async (item) => {
        try {
          ...
          return {
            ...item,
            impostos: res.impostosRs, // Usa valores em R$
            aliquotas: res.aliquotas,
            debugImposto: {
              percentuais: res.cardsPercent,
              brutoMotor: res.cards,
              baseCalculo: res?.debug?.input?.baseCalculo ?? null,
            },
            // NOVO: campos completos para salvar na dbitvenda
            campos: res.campos,
          };
                                ...
                                <DropdownMenuContent className="w-full">
                                  ...
                                  <DropdownMenuGroup>
                                    <DropdownMenuItem
                                      ...
                                      onSelect={async () => {
                                        setOpenFim(false);
                                        // Usa validação específica para salvar (permite vendas bloqueadas)
                                        const faltas =
                                          validarObrigatoriosParaSalvar();
                                        if (faltas.length > 0) {
                                          const faltas2 =
                                            validarObrigatoriosParaSalvar();
                                          ...
                                          setObrigPrazo(false); // Prazo não é obrigatório para salvar
                                          setObrigTransporte(
                                            faltas2.includes(
                                              'Tipo de transporte',
                                            ),
            ...
            <div className="mt-4 flex justify-end gap-2">
              {envioStep === 'ok' || envioStep === 'erro' ? (
                <>
                    ...
                    <>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          // ✅ FECHA modal de pendências e RESETA bloqueios
                          setOpenInfoCliente(false);
                          setBlocFin(false);
                          setBlocDesc(false);
                          setDescLiberado(false);
                          setFinLiberado(false);
                          setStatusVenda('VENDA LIBERADA');
                  ...
                  <button
                    className="px-3 w-40 h-9 rounded-md bg-blue-600 text-white hover:bg-blue-700"
                    onClick={() => {
                      try {
                        sessionStorage.removeItem('vendaDraftIdMelo');
                      } catch {}
