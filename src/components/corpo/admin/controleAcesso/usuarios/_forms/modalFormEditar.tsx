...
export default function FormEditarUsuario({
  ...
  // rótulo de uma conta pelo código (para exibir na coluna da tabela)
  ...
  const verificarAlteracoes = useCallback(() => {
    const gerarAssinatura = (perfis: typeof usuario.perfis) => {
      return perfis
        .map((p) => {
          const filiais = p.filial
            .map((f) => {
              const armazens = f.armazens
                ?.map((a) => a.id_armazem)
                .sort()
                .join(',');
              const funcoes = f.funcoesDoUsuario
                ?.map((fn) => fn.id_functions)
                .sort()
                .join(',');
              return `${f.codigo_filial}|${f.nome_filial}|${
                f.codvend ?? ''
              }|${f.codcomprador ?? ''}|${
                (f as { cod_conta?: string | null }).cod_conta ?? ''
              }|[${armazens}]|[${funcoes}]`;
            })
            .sort()
            .join(';');
          const funcoesPadrao = p.funcoesPadraoPerfil
            ?.map((f) => f.id_functions)
            .sort()
            .join(',');
          return `${p.perfil_name}|{${filiais}}|{${funcoesPadrao}}`;
        })
        .sort()
        .join('||');
    };

    const assinaturaOriginal = gerarAssinatura(usuario.perfis);
    const assinaturaAtual = gerarAssinatura(itensAdicionados.perfis);

    const perfisMudaram = assinaturaOriginal !== assinaturaAtual;
    const loginMudou = loginUser !== usuario.login_user_login;
    const nomeMudou = nomeUser !== usuario.login_user_name;

    setHasChanges(loginMudou || nomeMudou || perfisMudaram);
  }, [usuario, itensAdicionados, loginUser, nomeUser]);

  const isFirstVerificacao = useRef(true);

  useEffect(() => {
    if (isFirstVerificacao.current) {
      isFirstVerificacao.current = false;
      return; // Ignora primeira verificação
    }
  ...
  const handleAdicionarItem = () => {
    ...
    // Armazém só é obrigatório para quem VENDE (tem vendedor/codvend).
            ...
            <div className="flex gap-4 text-xs">
              <div className="w-1/3">
                ...
                <input
                  autoComplete="off"
                  type="text"
                  value={loginUser.toLocaleUpperCase()}
              ...
              <div className="w-1/3">
                ...
                <input
                  autoComplete="off"
                  type="text"
                  value={nomeUser.toLocaleUpperCase()}
              ...
              <div className="w-[25%]">
                <label className="block mb-1 text-sm font-medium">
                  ...
                  {perfilSelecionado === 'VENDAS' ? (
                    <span className="text-red-500 dark:text-red-300 text-xs mt-1">
                      (Obrigatório p/ Vendas)
                    </span>
                  ) : (
