...
interface LinhaCredor { credor: string; valores: Record<string, number>; total: number }
interface LinhaApi { grupo: string; centro: string; cfinanceira: string; credor: string; valores: Record<string, number>; total: number }
interface TituloDet {
  nota: string | null; dup: string | null; obs: string | null;
...
export default function ModalConsultaAvancadaPag({ isOpen, onClose, filial }: Props) {
  ...
  const [detalhe, setDetalhe] = useState<{ credor: string; cfinanceira: string; titulos: TituloDet[] } | null>(null);
  ...
  const consultar = async () => {
    ...
    try {
      ...
      const r = await fetch(`/api/contas-pagar/consulta-avancada-pagamentos?${qs}`);
  ...
  const abrirDetalhe = async (credor: string, cfinanceira: string) => {
    setCarregandoDet(true);
    setDetalhe({ credor, cfinanceira, titulos: [] });
    try {
      const qs = new URLSearchParams({ data_inicio: dataInicio, data_fim: dataFim, tipo_data: tipoData, detalhe: '1', credor, cfinanceira });
      const r = await fetch(`/api/contas-pagar/consulta-avancada-pagamentos?${qs}`);
      const d = await r.json();
      if (!r.ok) throw new Error(d.detalhes || d.erro || 'Erro no detalhe');
      setDetalhe({ credor, cfinanceira, titulos: d.titulos || [] });
    } catch (e: any) {
      toast.error(e.message);
      setDetalhe(null);
    } finally {
  ...
  // Árvore Grupo → Centro → CFinanceira → Fornecedor com subtotais por mês.
  const arvore = useMemo(() => {
    ...
    for (const l of linhas) {
      ...
      if (!cf) { cf = { nome: l.cfinanceira, valores: somaZero(), total: 0, credores: [] as LinhaCredor[] }; c.contas.set(l.cfinanceira, cf); }
      cf.credores.push({ credor: l.credor, valores: l.valores, total: l.total });
  ...
  const exportarExcel = async () => {
    try {
      ...
      const hr = ws.addRow(['', '', '', '', 'FORNECEDOR', ...meses.map(rotuloMesLongo), 'TOTAL']);
      ...
      for (const [, g] of arvore) {
        ...
        for (const [, c] of g.centros) {
          ...
          for (const [, cf] of c.contas) {
            ...
            for (const cd of cf.credores as LinhaCredor[]) {
              ws.addRow(['', '', '', '', cd.credor, ...meses.map((m) => Number(cd.valores[m] || 0)), cd.total]);
            }
          }
        }
      }
      ...
      baixarBlob(new Blob([buf], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }), `consulta-avancada-pagamentos-${hojeISO()}.xlsx`);
    } catch (e: any) {
  ...
  const exportarPdf = async () => {
    try {
      ...
      doc.text('CONSULTA AVANÇADA — PAGAMENTOS', 14, 12);
      ...
      for (const [, g] of arvore) {
        ...
        for (const [, c] of g.centros) {
          ...
          for (const [, cf] of c.contas) {
            ...
            for (const cd of cf.credores as LinhaCredor[]) {
              body.push([{ content: `        ${cd.credor}`, colSpan: 4 }, ...meses.map((m) => ({ content: brl(cd.valores[m] || 0), styles: { halign: 'right' } })), { content: brl(cd.total), styles: { halign: 'right' } }]);
            }
          }
        }
      }
      ...
      autoTable(doc, {
        startY: 21,
        head: [['Grupo / Centro / Conta / Fornecedor', '', '', '', ...meses.map(rotuloMes), 'Total']],
        body,
      ...
      doc.save(`consulta-avancada-pagamentos-${hojeISO()}.pdf`);
    } catch (e: any) {
    ...
    for (const [, g] of arvore) {
      ...
      for (const [, c] of g.centros) {
        ...
        for (const [, cf] of c.contas) {
          ...
          for (const cd of cf.credores as LinhaCredor[])
            linhasHtml.push(`<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${cd.credor}</td>${meses.map((m) => tdV(cd.valores[m] || 0)).join('')}${tdV(cd.total)}</tr>`);
        }
      }
    }
    linhasHtml.push(`<tr class="tg"><td>TOTAL GERAL</td>${meses.map((m) => tdV(totalGeral.valores[m])).join('')}${tdV(totalGeral.total)}</tr>`);
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Consulta Avançada — Pagamentos</title>
      <style>
      ...
      <h2>CONSULTA AVANÇADA — PAGAMENTOS</h2>
      ...
      <table><thead><tr><th>Grupo / Centro / Conta / Fornecedor</th>${meses.map((m) => `<th>${rotuloMes(m)}</th>`).join('')}<th>Total</th></tr></thead>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Consulta Avançada do Financeiro — Pagamentos" width="w-[98%] max-w-[1400px]">
      <div className="space-y-3">
        ...
        <div className="border border-gray-200 dark:border-slate-700 rounded overflow-auto max-h-[64vh]">
          <table className="w-full text-xs tabular-nums">
            <thead className="sticky top-0 bg-gray-100 dark:bg-slate-800 z-10">
              <tr>
                <th className="px-2 py-1.5 text-left min-w-[320px]">Grupo / Centro / Conta / Fornecedor</th>
            ...
            <tbody>
              ...
              {buscou && !carregando && linhas.length === 0 && <tr><td colSpan={colSpanTotal} className="px-3 py-10 text-center text-gray-400">Nenhum pagamento encontrado no período.</td></tr>}
                          ...
                          {cAberto && [...c.contas.values()].map((cf: any) => {
                            ...
                            return (
                              <FragmentRows key={kf}>
                                ...
                                {fAberto && (cf.credores as LinhaCredor[]).map((cd, i) => (
                                  <tr key={cd.credor + i} className="border-t border-gray-100 dark:border-slate-800 hover:bg-blue-50 dark:hover:bg-blue-950/30 cursor-pointer" onClick={() => abrirDetalhe(cd.credor, cf.nome)} title="Ver títulos">
                                    <td className="px-2 py-1 pl-12 text-blue-800 dark:text-blue-300">{cd.credor}</td>
                                    {meses.map((m) => <td key={m} className="px-2 py-1 text-right">{brl(cd.valores[m] || 0)}</td>)}
                                    <td className="px-2 py-1 text-right font-medium">{brl(cd.total)}</td>
                                  </tr>
                                ))}
                              </FragmentRows>
                            );
        ...
        <p className="text-[11px] text-gray-500">
          {linhas.length > 0 && `${linhas.length} linha(s). `}Clique num <b>fornecedor</b> para ver os títulos. Soma de <b>valor_pgto</b> por conta
          financeira (fiel ao Delphi FIN_PAGAMENSAL_DINAMICA). Unidade MAO.
        </p>
      </div>

      {/* Drill-down: títulos do fornecedor + conta financeira */}
      {detalhe && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4" onClick={() => setDetalhe(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xl w-full max-w-5xl max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-slate-700">
              <div className="text-sm font-semibold">
                {detalhe.credor} <span className="text-gray-400 font-normal">· {detalhe.cfinanceira}</span>
              </div>
              ...
              {carregandoDet ? (
                ...
                <table className="w-full text-xs tabular-nums">
                  <thead className="bg-gray-100 dark:bg-slate-800">
                    <tr>
                      <th className="px-2 py-1 text-left">NF</th><th className="px-2 py-1 text-left">Duplicata</th>
                      ...
                      <th className="px-2 py-1 text-right">Valor Pgto</th><th className="px-2 py-1 text-right">Valor Pago</th><th className="px-2 py-1 text-center">Pago</th>
                      <th className="px-2 py-1 text-left">Obs</th>
                    </tr>
                  </thead>
                  <tbody>
                    {detalhe.titulos.length === 0 && <tr><td colSpan={9} className="px-2 py-4 text-center text-gray-400">Sem títulos.</td></tr>}
                    {detalhe.titulos.map((t, i) => (
                      <tr key={i} className="border-t border-gray-100 dark:border-slate-800">
                        ...
                        <td className="px-2 py-1 max-w-[220px] truncate" title={t.obs || ''}>{t.obs || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
