...
interface UseRequisicoesTableImprovedProps {
  colunasIniciais?: string[];
  limiteColunas?: number;
  storageKey?: string;
}

export const useRequisicoesTableImproved = ({
  colunasIniciais,
  limiteColunas = 8,
  storageKey = 'requisicoes-table-config',
}: UseRequisicoesTableImprovedProps = {}) => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedConfig) {
        ...
        setHeaders(config.headers || getDefaultHeaders());
  ...
  const getDefaultHeaders = useCallback(() => {
    ...
    // Sempre incluir ações e seleção
    const headersPadrao = ['acoes', 'selecionar'];
    
    // Adicionar colunas fixas
    const colunasFixas = colunasDbRequisicao
      .filter(col => col.fixo && col.campo !== 'acoes' && col.campo !== 'selecionar')
      .map(col => col.campo);
    
    headersPadrao.push(...colunasFixas);

    // Adicionar colunas opcionais até o limite
    const colunasOpcionais = colunasDbRequisicao
      .filter(col => !col.fixo)
      .map(col => col.campo);
  ...
  const handleLimiteColunasChange = useCallback((novoLimite: number) => {
    ...
    const headersPadrao = getDefaultHeaders();
    const novosHeaders = headersPadrao.slice(0, novoLimite);
    
    setHeaders(novosHeaders);
    salvarConfiguracao(novosHeaders, novoLimite);
  }, [getDefaultHeaders, salvarConfiguracao]);
