...
import { loginSeparador, Separador } from '@/data/separacao/separacaoService';
import { useToast } from '@/hooks/use-toast';
import { Eye, EyeOff, User, Lock, Building } from 'lucide-react';

interface LoginSeparacaoProps {
  onLoginSuccess: (separador: Separador) => void;
}

const LoginSeparacao: React.FC<LoginSeparacaoProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!matricula.trim() || !codigoAcesso.trim()) {
      toast({
        ...
        description: 'Por favor, preencha matrícula e código de acesso.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      const separador = await loginSeparador(
        matricula.trim(),
        codigoAcesso.trim(),
      ...
      toast({
        ...
        description: `Bem-vindo(a), ${separador.nome}!`,
        variant: 'default',
      });

      onLoginSuccess(separador);
    } catch (error) {
      console.error('Erro no login:', error);
      toast({
        title: 'Erro no login',
        description: 'Matrícula ou código de acesso inválidos.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            Faça login para acessar o painel de separação
          </p>
        </div>
