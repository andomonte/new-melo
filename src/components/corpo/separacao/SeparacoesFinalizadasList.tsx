...
import { SeparacaoFinalizada } from '@/data/separacao/separacaoService';

interface SeparacoesFinalizadasListProps {
  separacoesFinalizadas: SeparacaoFinalizada[];
  isLoading: boolean;
}

/**
 * Componente para exibir lista de separações finalizadas
 *
 * Mostra um histórico das últimas separações concluídas pelo separador do dia atual,
 * com informações de tempo de separação e dados do pedido.
 */
const SeparacoesFinalizadasList: React.FC<SeparacoesFinalizadasListProps> = ({
  ...
  if (separacoesFinalizadas.length === 0) {
    // Debug log para entender o estado
    console.log('[DEBUG] SeparacoesFinalizadasList - Lista vazia:', {
      isLoading,
      arrayLength: separacoesFinalizadas.length,
      arrayType: typeof separacoesFinalizadas,
      arrayContent: separacoesFinalizadas,
    });
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-4">
        ...
        <div className="text-center py-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            Nenhuma separação finalizada recentemente.
          </p>
          <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">
            As separações finalizadas dos últimos 7 dias aparecerão aqui.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-3">
      ...
      <div className="space-y-2 max-h-80 overflow-y-auto">
        {separacoesFinalizadas.map((separacao) => (
          <div
            key={separacao.codvenda}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{formatarMoeda(separacao.total)}</span>
                  </div>

                  {separacao.tempoDeSeparacao && (
                    <div className="flex items-center gap-1">
                      <Clock className="w-3 h-3" />
                      <span>Tempo: {separacao.tempoDeSeparacao}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 flex-shrink-0 ml-4">
              <div className="text-right text-sm">
                <div className="flex items-center gap-1 text-gray-500 dark:text-gray-400 mb-1">
                  ...
                  <span>{formatarData(separacao.data)}</span>
                </div>
                <div className="text-xs text-gray-400 dark:text-gray-500">
                  Finalizado: {formatarFinalizacao(separacao.finalizadopedido)}
                </div>
              </div>
            </div>
          </div>
      ...
      {separacoesFinalizadas.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-zinc-600">
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            <span>
              Mostrando as {separacoesFinalizadas.length} separações mais
...
export default SeparacoesFinalizadasList;
