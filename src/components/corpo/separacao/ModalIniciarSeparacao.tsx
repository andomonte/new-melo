...
interface ModalIniciarSeparacaoProps {
  isOpen: boolean;
  onClose: () => void;
  separador: Separador;
  onSuccess: () => void;
...
const ModalIniciarSeparacao: React.FC<ModalIniciarSeparacaoProps> = ({
  isOpen,
  onClose,
  separador,
  onSuccess,
  ...
  // Usar o hook para tratamento de erros específicos
  const { tratarErroIniciarSeparacao } = useSeparacaoBusinessRules(
    separador.matricula,
  );

  // Dismiss any existing toasts when modal opens
  useEffect(() => {
    if (isOpen) {
      dismiss();
    }
  ...
  const handleButtonClick = async () => {
    ...
    try {
      const payload: IniciarSeparacaoPayload = {
        codVenda: codVenda.trim(),
        matriculaSeparador: separador.matricula,
        nomeSeparador: separador.nome,
      };

      await iniciarSeparacao(payload);

      toast({
        title: 'Separação iniciada',
        description: `Pedido ${codVenda} foi atribuído para separação.`,
        variant: 'default',
      });
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
          ...
          <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-4">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-blue-800 dark:text-blue-200">
                ...
                <p>
                  Digite o código da venda que você deseja separar. O pedido
        ...
        <DialogFooter className="flex gap-2">
          ...
          />
          <DefaultButton
            text={isLoading ? 'Iniciando...' : 'Iniciar Separação'}
            variant="primary"
...
export default ModalIniciarSeparacao;
