...
export default async function handler(
  ...
  try {
    ...
    const checkReq = await client.query(
      'SELECT req_id FROM cmp_requisicao WHERE req_id = $1 AND req_versao = $2',
      [reqId, reqVersao]
    );
    ...
    for (const produto of produtos) {
      ...
      const checkItem = await client.query(
        'SELECT itr_codprod FROM cmp_it_requisicao WHERE itr_req_id = $1 AND itr_req_versao = $2 AND itr_codprod = $3',
    ...
    for (const ordem of ordens) {
      ...
      const checkOrdem = await client.query(
        'SELECT orc_id FROM cmp_ordem_compra WHERE orc_id = $1',
        [ordem.id]
      );
