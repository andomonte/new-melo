...
async function hydrateRows(client: PoolClient, rows: any[]) {
  ...
  const itensFinalizadosQuery = ` 
    SELECT di.*, JSON_BUILD_OBJECT(
      ...
      'qtest', dp.qtest,
      'prvenda', dp.prvenda,
      'dbmarcas', JSON_BUILD_OBJECT('codmarca', dm.codmarca, 'descr', dm.descr)
    ) AS dbprod
    FROM dbitvenda di
  ...
  const draftsFullQuery = `
    SELECT CAST(draft_id AS text) AS codvenda, draft_id, filial, status, created_at, updated_at, expires_at, payload, codvend
    FROM dbvenda_draft
  ...
  // Map de drafts completos (inclui payload/header/prazos)
  ...
  rows.forEach((v: any) => {
    ...
    if (isDraft) {
      ...
      try {
        const hdr = v?.draft?.payload?.header ?? null;
        if (hdr && hdr.arm_id == null) {
          const ids = new Set(
            (Array.isArray(v.itens) ? v.itens : [])
              .map((it: any) => it?.arm_id)
              .filter((x: any) => x != null),
          );
          if (ids.size === 1) {
            v.draft.payload.header.arm_id = Array.from(ids)[0];
          }
        }
      } catch {}
      if (v.label == null && d?.label != null) v.label = d.label;
    } else {
      // Já existia: itens finalizados vindos da dbitvenda
      v.itens = itensMap.get(id) || [];
      v.dbitvenda = v.itens;

      // ✅ NOVO: montar um header no payload para VENDAS (dbvenda),
      // mimetizando a estrutura que a tela espera dos drafts (venda_draft)
      const cli = v.dbclien || null;
      const headerFromVenda = {
        codcli: v.codcli ?? null,
        nomecf:
          (cli?.nomefant != null && String(cli.nomefant)) ||
      ...
      // Envie em v.draft no mesmo "shape" básico usado por drafts
      v.draft = {
        draft_id: 'atualizar', // força recálculo de preços na nova venda (igual ao fluxo de copiar)
        payload: {
          header: headerFromVenda,
          // prazos: deixe ausente ou vazio; a tela já reconstrói quando necessário
          // itens: não é obrigatório aqui; o front lê itens de v.itens/dbitvenda.
        },
        // campos auxiliares alinhados ao que a tela costuma ler
        codcli: headerFromVenda.codcli,
        codusr: headerFromVenda.codusr,
        codvend: headerFromVenda.vendedor,
      };
    }
  });
