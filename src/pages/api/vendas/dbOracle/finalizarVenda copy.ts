...
type Prazo = {
  ...
  vencimento: string; // 'YYYY-MM-DD' ou Date
...
type Body = {
  header?: {
    ...
    bloqueada?: 'S' | 'N' | '0' | '1';
    estoque_virtual?: 'S' | 'N';
    uName?: string; // nome do usuário para logs no Oracle (opcional)
...
async function insertOracleItensAndStock(
  ...
  for (const it of itens) {
    ...
    // baixa estoque produto e armazém
    await conn.execute(
      `UPDATE DBPROD SET QTEST = QTEST - :QTD WHERE CODPROD = :CODPROD`,
      { QTD: it.qtd, CODPROD: it.codprod },
      { autoCommit: false },
    );
    await conn.execute(
      `UPDATE CAD_ARMAZEM_PRODUTO
          SET ARP_QTEST = ARP_QTEST - :QTD
...
async function insertPgItensAndStock(
  ...
  for (const it of itens) {
    ...
    // baixa estoque
    await client.query(
      `UPDATE ${schema}.dbprod SET qtest = qtest - $1 WHERE codprod = $2`,
      [it.qtd, it.codprod],
    );
    await client.query(
...
async function insertPgPrazosIfAny(
  client: PoolClient,
  schema: string,
  codvenda: string,
  ...
  // Detecta entre duas possibilidades de tabela
  const candidates = [
    {
      table: `${schema}.tb_pedido_user_parcelas`,
      cols: ['codvenda', 'parcela', 'vencimento', 'valor'],
      sql: `INSERT INTO ${schema}.tb_pedido_user_parcelas (codvenda, parcela, vencimento, valor)
            VALUES ($1,$2,$3,$4)`,
    },
    {
      table: `${schema}.tb_pedido_user`,
      cols: ['codvenda', 'parcela', 'vencimento', 'valor'],
      sql: `INSERT INTO ${schema}.tb_pedido_user (codvenda, parcela, vencimento, valor)
            VALUES ($1,$2,$3,$4)`,
    },
  ];

  // escolhe a primeira que existir com todas as colunas
  let chosen: { table: string; cols: string[]; sql: string } | undefined =
    undefined;
  ...
  for (const cand of candidates) {
    const q = await client.query(
      ...
      [schema, cand.table.split('.').pop(), cand.cols],
    );
    const n = Number(q.rows[0]?.n ?? 0);
    if (n >= cand.cols.length) {
      chosen = cand;
      break;
    }
  }
  ...
  if (!chosen) {
    throw new Error(
      'Tabela de parcelas/prazos não encontrada (tb_pedido_user_parcelas ou tb_pedido_user).',
    );
  }

  for (const p of prazos) {
    await client.query(chosen.sql, [
      codvenda,
      p.parcela,
      p.vencimento, // string 'YYYY-MM-DD' ou Date
      p.valor,
    ]);
  }
}
...
export default async function handler(
  ...
  try {
    ...
    await insertPgPrazosIfAny(pgClient, schema, ids.codvenda, body.prazos);
