...
type ItemPayload = {
  ...
  // --- Kickback ---
  kickback?: boolean | 'S' | 'N' | null;
  usandoKickback?: boolean;
};
...
async function insertOracleItensAndStock(
  ...
  for (const it of itens) {
    ...
    // Determina se o item está usando kickback
    const itemKickback =
      it.kickback === true ||
      it.kickback === 'S' ||
      it.usandoKickback === true
        ? 'S'
        : 'N';
    ...
    await conn.execute(
      `INSERT INTO DBITVENDA (
         ...
         CODVEND, CODOPERADOR, PRCOMPRA, PRMEDIO, DESCONTO, NREQUI, NRITEM, ARM_ID, REF, KICKBACK
       ) VALUES (
         :CODVENDA, :CODPROD, :PRUNIT, :QTD, 'S', :DESCR, NULL, NULL,
         NULL, NULL, :PRCOMPRA, :PRMEDIO, :DESCONTO, NULL, :NRITEM, :ARM, :REF, :KICKBACK
       )`,
      {
        CODVENDA: ids.codvenda,
        CODPROD: it.codprod,
