...
export default async function handler(
  ...
  try {
    ...
    for (const item of itens) {
      ...
      // Buscar dados do produto (ref e alíquotas)
      if (codprod) {
        const queryProd = `
          SELECT
            p.ref,
        ...
        if (resultProd.rows.length > 0) {
          ...
          aliquotas = {
            icms: 18, // ICMS padrão para AM (será ajustado conforme operação)
    ...
    if (logoBase64) {
      ...
      // Título ORÇAMENTO à direita do logo
      doc.setFontSize(20);
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        4: { cellWidth: 22, halign: 'right' }, // PREÇO UNIT
        5: { cellWidth: 14, halign: 'center' }, // ICMS%
    ...
    // Total do orçamento (à esquerda do box)
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    ...
    const totalComImpostos = dadosOrcamento.total + dadosOrcamento.total_ipi; // IPI agrega ao total
    ...
    if (dadosOrcamento.obs) {
      ...
      doc.text(obsLines.slice(0, 3), 14, obsY + 5); // Máximo 3 linhas
    }
    ...
    // Enviar PDF (em memória, não salva em disco)
    const pdfData = doc.output('arraybuffer');
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader(
      'Content-Disposition',
      `inline; filename=orcamento_${dadosOrcamento.codvenda}.pdf`,
    );
