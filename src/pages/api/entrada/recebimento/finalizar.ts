...
export default async function handler(
  ...
  try {
    ...
    const divergenciasResult = await client.query(
      `SELECT COUNT(*) as divergencias FROM entrada_itens_recebimento
    ...
    await client.query(
      `UPDATE dbent_recebimento SET status = 'RECEBIDO', updated_at = now() WHERE codent = $1`,
      [entradaId]);

    // Romaneio automático (se não houver)
    const romaneioResult = await client.query(`SELECT COUNT(*) as total FROM dbitent_armazem WHERE codent = $1`, [entradaId]);
    if (parseInt(romaneioResult.rows[0].total) === 0) {
      const itensResult = await client.query(`SELECT codprod, codreq, quant FROM dbitent WHERE codent = $1`, [entradaId]);
    ...
    if (temDivergencia) {
      const itensFalta = await client.query(
        `SELECT eir.id as eir_id, eir.produto_cod,
           ...
           LEFT JOIN dbprod p ON p.codprod = eir.produto_cod
      ...
      if (itensFalta.rows.length > 0) {
        const entDados = await client.query(
          `SELECT COALESCE(em.xnome, '') as fornecedor,
             ...
             FROM dbent e
             LEFT JOIN dbnfe_ent ne ON ne.chave = e.chave
    ...
    // Log da operação
    await client.query(
      `INSERT INTO entrada_operacoes_log (codent, operacao, status_anterior, status_novo, observacao, created_at)
