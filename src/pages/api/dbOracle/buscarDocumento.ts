...
export default async function Sec(
  ...
  try {
    ...
    const querySql = `
      SELECT
          cod_operacao, -- Nome da coluna em minúsculas
          descr         -- Nome da coluna em minúsculas
      FROM dboperacao_venda -- Nome da tabela em minúsculas
    ...
    res
      .status(500)
      .json({ error: 'ERRO AO BUSCAR DADOS DE OPERAÇÃO DE VENDA' });
  } finally {
    if (client) {
