...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const uso = await client.query('SELECT 1 FROM db_manaus.dbprod WHERE codgpf = $1 LIMIT 1', [codgpf]);
    if (uso.rowCount)
      return res.status(400).json({ error: 'Não é possível excluir: há produtos usando este grupo de função.' });
    const r = await client.query('DELETE FROM db_manaus.dbgpfunc WHERE codgpf = $1', [codgpf]);
