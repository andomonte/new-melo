...
export default async function handle(
  ...
  try {
    ...
    // Buscar os cests
    const cestsResult = await client.query(
      `SELECT * FROM dbcest ${whereClause} ORDER BY ncm ${limitOffset}`,
      queryParams,
    );
    ...
    const countResult = await client.query(
      `SELECT COUNT(*) as total FROM dbcest ${whereClause}`,
      countParams,
    );

    const cests = cestsResult.rows;
    const count = parseInt(countResult.rows[0].total, 10);

    res.status(200).json({
      data: cests.map((cest) => serializeBigInt(cest)),
      meta: {
        total: count,
