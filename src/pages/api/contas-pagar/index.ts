...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Filtro por conta internacional
    if (eh_internacional) {
      whereClause += ` AND p.eh_internacional = $${paramIndex}`;
      params.push(eh_internacional);
      paramIndex++;
    }
    ...
    // Busca geral (ID, credor, NF, duplicata)
    if (search) {
      const searchParamStart = paramIndex;
      whereClause += ` AND (
        CAST(p.cod_pgto AS TEXT) LIKE $${paramIndex}
        OR UPPER(c.nome) LIKE UPPER($${paramIndex + 1})
        OR p.nro_nf LIKE $${paramIndex + 2}
        OR p.nro_dup LIKE $${paramIndex + 3}
        OR p.obs LIKE $${paramIndex + 4}
      )`;
      params.push(`%${search}%`);
      params.push(`%${search}%`);
      params.push(`%${search}%`);
    ...
    // Por padrão, ocultar títulos cancelados (a menos que seja explicitamente filtrado)
