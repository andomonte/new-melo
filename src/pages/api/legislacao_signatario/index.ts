...
const handleGetList = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    // ✅ ADAPTADO:
    const totalQuery = `SELECT COUNT(*) FROM "CAD_LEGISLACAO_SIGNATARIO" ${whereClause}`;
    const totalResult = await client.query(totalQuery, searchParam);
    const total = parseInt(totalResult.rows[0].count, 10);

    // ✅ ADAPTADO:
    const dataQuery = `
      SELECT * FROM "CAD_LEGISLACAO_SIGNATARIO"
      ${whereClause}
...
const handleCreate = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const nextIdQuery = `SELECT COALESCE(MAX("LES_ID"), 0) + 1 as next_id FROM "CAD_LEGISLACAO_SIGNATARIO"`;
    const nextIdResult = await client.query(nextIdQuery);
    const nextId = nextIdResult.rows[0].next_id;

    const query = `
      INSERT INTO "CAD_LEGISLACAO_SIGNATARIO" (
