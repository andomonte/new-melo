...
export default async function handler(
  ...
  if (body.filial) {
    console.log(`Login separação usando filial do request: ${filial}`);
  } else if (cookies.filial_melo) {
    console.log(`Login separação usando filial do cookie: ${filial}`);
  } else {
    console.log(
      'Login separação usando filial padrão (MANAUS) - sem cookie ou filial no request',
    );
  }
  ...
  try {
    ...
    console.log('Tentativa de login para separação:', {
    ...
    if (result.rows.length === 0) {
      ...
      console.warn('Falha na autenticação para separação:', {
    ...
    // Success audit log
    console.log('Login realizado com sucesso para separação:', {
    ...
    console.error('Erro crítico no login para separação:', {
      matricula: matricula ? matricula.substring(0, 3) + '***' : 'undefined',
      filial,
      error:
