...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (chave) {
      const r = await client.query('SELECT chave, valor, tipo FROM db_manaus.tb_parametros WHERE chave = $1', [chave]);
    ...
    const r = await client.query('SELECT chave, valor, tipo, descricao FROM db_manaus.tb_parametros ORDER BY chave');
