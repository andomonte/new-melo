...
export default async function handle(
  ...
  try {
    ...
    const whereClause = search ? 'WHERE descr ILIKE $1' : '';
    ...
    // Buscar as classes de clientes
    const classesResult = await client.query(
      `SELECT * FROM dbcclien ${whereClause} ORDER BY CAST(codcc AS INTEGER) ${limitOffset}`,
      queryParams,
    );
    ...
    const countResult = await client.query(
      `SELECT COUNT(*) as total FROM dbcclien ${whereClause}`,
      countParams,
    );
    ...
    res
      ...
      .json({
        data: classes.map((classe) => serializeBigInt(classe)),
        meta: {
          total: count,
    ...
    console.error('Erro ao buscar classes de clientes:', error);
