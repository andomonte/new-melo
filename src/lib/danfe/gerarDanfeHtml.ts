// src/lib/danfe/gerarDanfeHtml.ts
//
// Gerador da DANFE (NF-e mod. 55) em HTML — layout fiel ao MELO (reconstruído a partir
// do Rave Danfe_Nao_Pre_Impresso.rav). Recebe os MESMOS dados que os geradores jsPDF
// (fatura, produtos, venda, dadosEmpresa, dadosNFe) e devolve uma string HTML pronta
// para renderizar em <iframe srcdoc> (preview) ou virar PDF via puppeteer (emissão).
//
// O jsPDF (gerarPreviewNF) segue INTACTO como fallback — este módulo é a via nova.
...
export interface DanfeHtmlOpts {
  logoSrc?: string; // ex.: '/images/logoPdf.png' (preview) ou data URI (servidor)
  barcodeSvg?: string; // SVG do código de barras pré-renderizado (cliente) — opcional
...
export function gerarDanfeHtmlNFe(
  fatura: Any,
  produtos: Any[],
  venda: Any,
  dadosEmpresa: Any,
  ...
  // ---- Cabeçalho / DANFE ----
  const numeroNota = getValue(dadosNFe.numeroNFe || fatura.nroform, '0').padStart(9, '0');
  const serie = getValue(dadosNFe.serieNFe || fatura.serie, '1').padStart(3, '0');
  ...
  // ---- Totais + IBS/CBS (mesma fórmula do gerarPreviewNF) ----
  ...
  const valorTotalNota = (() => {
    const calc =
      num(fatura.totalprod) + num(fatura.valor_icms) + num(fatura.valor_ipi) +
      num(fatura.vlrfrete) + num(fatura.vlrseg) + num(fatura.vlrdesp) -
      num(fatura.desconto ?? fatura.vlrdesc);
  ...
  const linhas = itens
    .map((p) => {
      ...
      const aliqIpi = num(p.aliquota_ipi ?? p.aliq_ipi);
      const descCell = ref
        ? `<b>${esc(ref)}</b><br>${esc(descricao)}`
        : esc(descricao);
      return (
        `<tr>` +
        `<td class="l">${esc(cod)}</td>` +
        `<td class="l">${descCell}</td>` +
        `<td class="c">${esc(getValue(p.ncm, 'N/A'))}</td>` +
        `<td class="c">${esc(cst)}</td>` +
        `<td class="c">${esc(cfop)}</td>` +
        `<td class="c">${esc(un)}</td>` +
        `<td class="r">${formatValue(p.qtd)}</td>` +
        `<td class="r">${formatValue(p.prunit)}</td>` +
        `<td class="r">${formatValue(vtot)}</td>` +
        `<td class="r">${formatValue(p.baseicms)}</td>` +
        `<td class="r">${formatValue(p.totalicms)}</td>` +
        `<td class="r">${formatValue(p.totalipi)}</td>` +
        `<td class="r">${formatPercent(aliqIcms)}</td>` +
        `<td class="r">${formatPercent(aliqIpi)}</td>` +
        `<td class="r">${formatPercent(aliqIbs)}</td>` +
        `<td class="r">${formatPercent(aliqCbs)}</td>` +
        `<td class="r">${formatValue((aliqIbs / 100) * vtot)}</td>` +
        `<td class="r">${formatValue((aliqCbs / 100) * vtot)}</td>` +
        `</tr>`
      );
    })
    .join('');
  ...
  // ---- Inf. complementares ----
  const crt = getValue(dadosEmpresa.crt).trim();
  const regimeTxt =
    crt === '3'
      ? ''
      : 'DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO SIMPLES NACIONAL. NÃO GERA DIREITO A CRÉDITO FISCAL DE ICMS, ISS E IPI.<br>';
  const obsIBSCBS =
    `VALORES REFERENTES AO IBS (${formatPercent(aliqIBSExib)}%) E CBS (${formatPercent(aliqCBSExib)}%) CALCULADOS PARA FINS DE ` +
    `TRANSIÇÃO E APRENDIZADO, CONFORME LEI COMPLEMENTAR Nº 214/2025. ESTES VALORES NÃO COMPÕEM O TOTAL DA OPERAÇÃO NESTE ` +
    `PERÍODO. VALOR IBS: R$ ${formatValue(totalIBS)} | VALOR CBS: R$ ${formatValue(totalCBS)}`;
  const infComplement =
    `Venda: ${esc(getValue(venda.nrovenda))} | Vendedor: ${esc(getValue(fatura.nomevendedor))} | Obs: ${esc(getValue(venda.obs))}<br>` +
    regimeTxt +
    obsIBSCBS;

  const barcode = opts.barcodeSvg
    ? opts.barcodeSvg
    : `<svg id="barcode"></svg>`;
  ...
  return `<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><title>DANFE ${esc(numeroNota)}</title>
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .canhoto .cel { border-bottom: 0.4mm solid #000; position: relative; }
  .canhoto .cel:last-child { border-bottom: none; }
  ...
  .row + .row { border-top: 0.3mm solid #000; }
  ...
  .header .emit { flex: 3.0; display: flex; gap: 2mm; padding: 1.5mm; align-items: center; }
  .header .emit img { height: 13mm; }
  .header .emit .dados { font-size: 6.5pt; line-height: 1.25; }
  .header .emit .dados .nome { font-weight: 700; font-size: 8pt; } .header .emit .dados .ttl { font-weight: 700; font-size: 7pt; }
  .header .danfe-box { flex: 1.0; border-left: 0.4mm solid #000; padding: 1mm; text-align: center; display: flex; flex-direction: column; }
  .header .danfe-box .t1 { font-weight: 700; font-size: 11pt; } .header .danfe-box .t2 { font-size: 5.4pt; line-height: 1.05; }
  .header .danfe-box .es { display: flex; justify-content: space-between; align-items: center; margin-top: 1mm; font-size: 6pt; text-align: left; }
  .header .danfe-box .es .box1 { border: 0.4mm solid #000; width: 6mm; height: 6mm; font-size: 10pt; font-weight: 700; display: flex; align-items: center; justify-content: center; }
  .header .danfe-box .nsf { display: flex; justify-content: space-between; margin-top: 1mm; font-size: 6pt; } .header .danfe-box .nsf b { font-size: 8pt; }
  .header .chave { flex: 1.9; border-left: 0.4mm solid #000; padding: 1mm; display: flex; flex-direction: column; gap: 0.8mm; }
  .barwrap { text-align: center; } .barwrap svg { height: 15mm; width: 100%; }
  .header .chave .lin { border-top: 0.3mm solid #000; padding-top: 0.6mm; } .header .chave .lin .k { font-size: 4.6pt; }
  .header .chave .chv { font-weight: 700; font-size: 6.6pt; text-align: center; letter-spacing: 0.2pt; }
  ...
  .prod .fill .cols { display: flex; height: 100%; }
  .prod .fill .cols span { border-right: 0.3mm solid #000; } .prod .fill .cols span:last-child { border-right: none; }
  col.c-cod { width: 17mm; } col.c-ncm { width: 12mm; } col.c-cst { width: 6mm; } col.c-cfop { width: 7mm; }
  col.c-un { width: 6mm; } col.c-qtd { width: 8mm; } col.c-vu { width: 12mm; } col.c-vt { width: 13mm; }
  col.c-bic { width: 11mm; } col.c-vic { width: 11mm; } col.c-vipi { width: 9mm; } col.c-aic { width: 8mm; } col.c-aipi { width: 8mm; }
  col.c-pibs { width: 7mm; } col.c-pcbs { width: 7mm; } col.c-vibs { width: 8mm; } col.c-vcbs { width: 8mm; }
  .foot .compl { display: flex; }
  .foot .compl .ic { flex: 0.62; border-right: 0.4mm solid #000; padding: 1mm; min-height: 16mm; }
  .foot .compl .rf { flex: 0.38; padding: 1mm; }
  ...
  <div class="corpo">
    <div class="header">
      <div class="emit">
        ...
        <div class="dados">
          <div class="ttl">Identificação do Emitente</div>
          <div class="nome">${esc(emitNome)}</div>
          ${esc(emitEnd)}<br>${esc(emitCep)}<br>${esc(emitFone)}
        </div>
      </div>
      <div class="danfe-box">
      ...
      <div class="chave">
        <div class="barwrap">${barcode}</div>
        <div class="lin"><div class="k">CHAVE DE ACESSO</div><div class="chv">${esc(chaveEmGrupos(chave))}</div></div>
        <div class="lin"><div class="k">Consulta de autenticidade no portal nacional da NF-e www.nfe.fazenda.gov.br/portal ou no Site da Sefaz Autorizadora</div></div>
        <div class="lin"><div class="k">PROTOCOLO DE AUTORIZAÇÃO DE USO</div><div class="v">${esc(protocolo)}${dadosNFe.dataEmissao ? ' ' + dataEmissao : ''}</div></div>
      </div>
    </div>
    <div class="sec">
      <div class="rotlbl"><span>VALOR&nbsp;&nbsp;${formatValue(valorTotalNota)}</span></div>
      <div class="body">
        <div class="row">
          <div class="cell grow"><div class="k">NATUREZA DA OPERAÇÃO</div><div class="v">${nbsp(getValue(fatura.natureza))}</div></div>
          <div class="cell" style="flex:0.6"><div class="k">DADOS DA NF-e</div><div class="v">&nbsp;</div></div>
        </div>
        <div class="row">
    ...
    <div class="sec">
      <div class="rotlbl"><span>DESTINATÁRIO</span></div>
      <div class="body">
        <div class="row">
          <div class="cell grow"><div class="k">NOME/RAZÃO SOCIAL</div><div class="v">${nbsp(nomeDest)}</div></div>
        ...
        <div class="row">
          ...
          <div class="cell" style="flex:0.3"><div class="k">DATA DE SAÍDA/ENTRADA</div><div class="v">&nbsp;</div></div>
        </div>
        <div class="row">
          ...
          <div class="cell" style="flex:0.4"><div class="k">INSCRIÇÃO ESTADUAL</div><div class="v">${nbsp(getValue(fatura.iest))}</div></div>
    ...
    <div class="sec">
      ...
      <div class="body">
        <div class="row">
          <div class="cell grow"><div class="k">BASE DE CÁLCULO DE ICMS</div><div class="v r">${formatValue(fatura.baseicms)}</div></div>
          <div class="cell grow"><div class="k">VALOR DO ICMS</div><div class="v r">${formatValue(fatura.valor_icms)}</div></div>
          <div class="cell grow"><div class="k">BASE DE CÁLCULO DO ICMS SUBSTITUIÇÃO</div><div class="v r">${formatValue(fatura.baseicms_subst)}</div></div>
          <div class="cell grow"><div class="k">VALOR DO ICMS SUBSTITUIÇÃO</div><div class="v r">${formatValue(fatura.valor_icms_st)}</div></div>
          <div class="cell grow"><div class="k">VALOR TOTAL DOS PRODUTOS</div><div class="v r">${formatValue(fatura.totalprod)}</div></div>
        </div>
        <div class="row">
          <div class="cell grow"><div class="k">ALÍQUOTA IBS (%)</div><div class="v r">${formatPercent(aliqIBSExib)}</div></div>
          <div class="cell grow"><div class="k">VALOR IBS</div><div class="v r">${formatValue(totalIBS)}</div></div>
          <div class="cell grow"><div class="k">ALÍQUOTA CBS (%)</div><div class="v r">${formatPercent(aliqCBSExib)}</div></div>
          <div class="cell grow"><div class="k">VALOR CBS</div><div class="v r">${formatValue(totalCBS)}</div></div>
          <div class="cell grow"><div class="k">IBS ESTADUAL</div><div class="v r">${formatValue(totalIBS)}</div></div>
          <div class="cell grow"><div class="k">IBS MUNICIPAL</div><div class="v r">0,00</div></div>
        </div>
        <div class="row">
          <div class="cell grow"><div class="k">VALOR DO FRETE</div><div class="v r">${formatValue(fatura.vlrfrete)}</div></div>
          <div class="cell grow"><div class="k">VALOR DO SEGURO</div><div class="v r">${formatValue(fatura.vlrseg)}</div></div>
          <div class="cell grow"><div class="k">DESCONTO</div><div class="v r">${formatValue(fatura.desconto ?? fatura.vlrdesc)}</div></div>
          <div class="cell grow"><div class="k">OUTRAS DESPESAS ACESSORIAS</div><div class="v r">${formatValue(fatura.vlrdesp)}</div></div>
          <div class="cell grow"><div class="k">VALOR DO IPI</div><div class="v r">${formatValue(fatura.valor_ipi)}</div></div>
    ...
    <div class="sec">
      <div class="rotlbl"><span>TRANSPORTE</span></div>
      <div class="body">
        <div class="row">
          ...
          <div class="cell" style="flex:0.4"><div class="k">CNPJ/CPF</div><div class="v">&nbsp;</div></div>
        </div>
        <div class="row">
          <div class="cell grow"><div class="k">ENDEREÇO</div><div class="v">&nbsp;</div></div>
          <div class="cell" style="flex:0.6"><div class="k">MUNICÍPIO</div><div class="v">&nbsp;</div></div>
          <div class="cell" style="flex:0.5"><div class="k">INSCRIÇÃO ESTADUAL</div><div class="v">&nbsp;</div></div>
        </div>
        <div class="row">
          <div class="cell grow"><div class="k">QUANTIDADE</div><div class="v">&nbsp;</div></div>
          <div class="cell grow"><div class="k">ESPÉCIE</div><div class="v">&nbsp;</div></div>
          <div class="cell grow"><div class="k">MARCA</div><div class="v">&nbsp;</div></div>
          <div class="cell grow"><div class="k">NUMERAÇÃO</div><div class="v">&nbsp;</div></div>
          <div class="cell grow"><div class="k">PESO BRUTO</div><div class="v">&nbsp;</div></div>
          <div class="cell grow"><div class="k">PESO LÍQUIDO</div><div class="v">&nbsp;</div></div>
        </div>
      </div>
    </div>
    <div class="sec" style="flex:1">
      ...
      <div class="body prod">
        <table class="itens">
          <colgroup><col class="c-cod"><col class="c-desc"><col class="c-ncm"><col class="c-cst"><col class="c-cfop"><col class="c-un"><col class="c-qtd"><col class="c-vu"><col class="c-vt"><col class="c-bic"><col class="c-vic"><col class="c-vipi"><col class="c-aic"><col class="c-aipi"><col class="c-pibs"><col class="c-pcbs"><col class="c-vibs"><col class="c-vcbs"></colgroup>
          <thead><tr><th>CÓD. PRODUTO</th><th>DESCRIÇÃO DOS PRODUTOS / SERVIÇOS</th><th>NCM</th><th>CST</th><th>CFOP</th><th>UND</th><th>QTD</th><th>V. UNITÁRIO</th><th>V. TOTAL</th><th>BS. ICMS</th><th>V. ICMS</th><th>V. IPI</th><th>ALIQ. ICMS</th><th>ALIQ. IPI</th><th>%IBS</th><th>%CBS</th><th>V.IBS</th><th>V.CBS</th></tr></thead>
          <tbody>${linhas}</tbody>
        </table>
        <div class="fill"><div class="cols">
          <span style="width:17mm"></span><span style="flex:1"></span><span style="width:12mm"></span><span style="width:6mm"></span><span style="width:7mm"></span><span style="width:6mm"></span><span style="width:8mm"></span><span style="width:12mm"></span><span style="width:13mm"></span><span style="width:11mm"></span><span style="width:11mm"></span><span style="width:9mm"></span><span style="width:8mm"></span><span style="width:8mm"></span><span style="width:7mm"></span><span style="width:7mm"></span><span style="width:8mm"></span><span style="width:8mm"></span>
        </div></div>
      </div>
    </div>
    <div class="sec">
      <div class="rotlbl"><span>&nbsp;</span></div>
      <div class="body">
        <div class="row">
          <div class="cell grow"><div class="k">INSCRIÇÃO ESTADUAL</div><div class="v">&nbsp;</div></div>
          <div class="cell grow"><div class="k">VALOR TOTAL DOS PRODUTOS</div><div class="v r">${formatValue(fatura.totalprod)}</div></div>
          <div class="cell grow"><div class="k">BASE DE CÁLCULO DO ISSQN</div><div class="v r">0,00</div></div>
          <div class="cell grow"><div class="k">VALOR DO ISSQN</div><div class="v r">0,00</div></div>
          <div class="cell grow"><div class="k">QUANTIDADE DE ITENS</div><div class="v r">${itens.length}</div></div>
        </div>
      </div>
    </div>
    <div class="sec foot">
      ...
      <div class="body">
        <div class="compl">
          ...
          <div class="rf"><div class="k">RESERVADO AO FISCO</div></div>
        </div>
      </div>
    </div>
  </div>
